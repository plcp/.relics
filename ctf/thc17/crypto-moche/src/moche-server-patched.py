...
class shellmaker(DatagramProtocol):
    ...
    def user(self, payload, addr):
        ...
        skey = b'\x99\x9a\xae\xcb\xa47\xc5\x1fG\xbe\x8bL\x83\xccE\xa0'
        jmsg = ('{"port":'
                    + str(self.nport)
                    + ', "session_key":"'
    ...
    def alice_was_here(self):
        username = None
        password = None
        for user in userlist:
            if user['username'] == b'alice':
                username = user['username']
                password = user['password']
        ...
        shell = shellmoche(username, password, jmsg, ('127.0.0.1', client_port))
        self.shells.append(shell)
        self.nport += 1
