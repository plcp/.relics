...
def accept(client, addr):
    ...
    for i in range(128):
        motd = (b"\nLet's"
            + b' prove ourselves that computers do have free will: '
        ...
        hchl = crypto.hash(chl)[:4]
        if hchl in history:
            client.sendall(b'\n...you can not ask the same question twice!\n')
            return
        else:
            history.append(hchl)
        ...
        ans = crypto.cbc.encrypt(key, chl)
        if pick:
            ans = os.urandom(len(ans))
        client.sendall(binascii.hexlify(ans))

        answers.append(pick)
        ciphers.append(ans)

    while True:
        motd = b'\nYou can now query the meta-oracle: '
        client.sendall(motd)

        client.sendall(b'\n - Please send 48 bytes: ')
        chl = binascii.unhexlify(client.recv(97)[:-1])
        if len(chl) != 48:
            client.sendall(b'!! Expected format: 96 nibbles + \\x0a\n')
            return

        hchl = crypto.hash(chl)[:4]
        if hchl in ciphers:
            client.sendall(b'\n...the meta-oracle says:' +
                b' You can not ask a previously given answer!\n')
            return

        client.sendall(b' - Here is the truth: ')
        client.sendall(binascii.hexlify(crypto.cbc.decrypt(key, chl)))

        client.sendall(b'\n - Ready to give your answers?')
        client.sendall(b'\n   - Yes (0)')
        client.sendall(b'\n   - No  (1) ')
        if b'0' == client.recv(1):
            break
    ...
    for i in range(128):
        ...
        client.sendall(b'\n   - entropy     (0)')
        client.sendall(b'\n   - ciphertext  (1) ')
        if int(b'1' == client.recv(1)) ^ int(not answers[i]) != 0:
            client.sendall(b'\n...eh, told ya!\n\nYour oracle failed, ')
