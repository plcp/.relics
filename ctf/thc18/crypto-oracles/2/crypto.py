...
class ctr:
    @staticmethod
    def encrypt(key, msg, nsize=16):
        nounce = os.urandom(nsize)
        if nsize < 16:
            nce = bytes(16 - nsize) + nounce
        elif nsize > 16:
            nce = stretch(nounce, 16, salt=bytes(16))
        else:
            nce = nounce

        ctx = Cipher(algorithms.AES(key), modes.CTR(nce),
                backend=default_backend()).encryptor()
        return nounce + ctx.update(msg) + ctx.finalize()

    @staticmethod
    def decrypt(key, msg, nsize=16):
        nounce = msg[:nsize]
        if nsize < 16:
            nce = bytes(16 - nsize) + nounce
        elif nsize > 16:
        ...
        ctx = Cipher(algorithms.AES(key), modes.CTR(nce),
                backend=default_backend()).decryptor()
        return ctx.update(msg[nsize:]) + ctx.finalize()
